...
** Introduction 
   ...
   *n-dimensional array*.  
   ...
   information:
    ...
    2. Event table describing reconstructed and simulated shower
       parameters ([[EVENTS]])
    3. Optional Telescope-Event table with low-level parameters ([[TEVENTS]])
    4. Array description table ([[TELARRAY]])
    5. Monte-Carlo information Headers
    6. Monte-Carlo thrown energy distribution histogram ([[MCENERGY]])
